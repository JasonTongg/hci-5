...
navbar.addEventListener("mouseover", (e) => {
    document.querySelectorAll(".navbar--button").forEach((item) => item.style.opacity= ".5");
    document.querySelector(".navbar--image").style.opacity = ".5";
    if(e.target.classList.contains("navbar--button") || e.target.classList.contains("navbar--image")){
        e.target.style.opacity = "1";
    }
})

navbar.addEventListener("mouseout", (e) => {
    document.querySelectorAll(".navbar--button").forEach((item) => item.style.opacity= "1");
...
//Operation Page
let operationInfoContainer = document.querySelector(".operation__items--infoContainer");
let operationInfo = document.querySelector(".operation__items--info");

operationInfoContainer.style.height = `${operationInfo.getBoundingClientRect().height}px`;
...
let contentContainer = document.querySelector(".operation__items--infoContainer");
let content = document.querySelectorAll(".operation__items--info");


tab.forEach(item => item.addEventListener("click", (e) => {
    tab.forEach(items => items.classList.remove("btn-active"));
    e.target.classList.add("btn-active");

    content.forEach(items => items.classList.remove("info--active"));
    document.querySelector(`.info--${e.target.dataset.number}`).classList.add("info--active");

}))

//testimoni page
let testimoniItemsInfoContainer = document.querySelector(".testimoni__items--infoContainer");
let testimoniItemsInfo = document.querySelector(".testimoni__items--info");

testimoniItemsInfoContainer.style.height = `${testimoniItemsInfo.getBoundingClientRect().height}px`;
...
let sectionAnimation = (entries, observer) => {
    ...
    entrie.target.classList.remove("sectionHidden");
